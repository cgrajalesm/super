...
def send_email(to_emails, subject, body, attachment_path=None):
    ...
    Args:
        ...
        attachment_path: Ruta opcional del archivo adjunto
    """
    print(f"\n{'='*70}")
    ...
    if attachment_path:
        ...
        if not os.path.exists(attachment_path):
            print(f"❌ ERROR: El archivo adjunto NO existe: {attachment_path}")
            print(f"{'='*70}\n")
            return
        else:
    ...
    try:
        ...
        if attachment_path:
            with open(attachment_path, 'rb') as attachment:
                ...
                msg.attach(part)
            print(f"✓ Archivo adjunto agregado al mensaje")
        ...
        print(f"   Verifica las credenciales: {EMAIL_USER}")
        print(f"{'='*70}\n")
...
if all_data:
    ...
    print(f"Cantidad: {len(existing_sheets)}")

    # Only create temp file if there are existing sheets to include
    if existing_sheets:
        print(f"✓ Se encontraron hojas válidas. Procediendo con el correo PowerAutomate...")
        print(f"{'='*70}\n")
    else:
        print(f"❌ NO se encontraron hojas válidas.")
        print(f"   El correo 'Inasistencias - PowerAutomate' NO será enviado.")
        print(f"{'='*70}\n")
