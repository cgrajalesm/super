...
# Function to send email (DEBUG MODE - NO SENDING)
def send_email(to_emails, subject, body, attachment_path=None):
    print(f"\n{'='*60}")
    print(f"DEBUG: send_email llamado")
    print(f"{'='*60}")
    ...
    if subject == "Inasistencias - PowerAutomate":
        print("\n⚠️ ESTE ES EL EMAIL CRITICO QUE EL USUARIO PREGUNTA ⚠️")
        print(f"Archivo adjunto: {attachment_path}")
        if attachment_path and os.path.exists(attachment_path):
            print(f"✓ Archivo adjunto existe: {attachment_path}")
        elif attachment_path:
            print(f"✗ PROBLEMA: Archivo adjunto NO existe: {attachment_path}")
...
if all_data:
    ...
    print("\n" + "="*70)
    print("DEBUG: Sección crítica - Email PowerAutomate")
    print("="*70)
    print(f"Hojas buscadas (selected_sheets): {selected_sheets}")
    print(f"Hojas disponibles en el Excel (dfs.keys()): {list(dfs.keys())}")

    # Filter only sheets that exist in the workbook
    existing_sheets = [sheet for sheet in selected_sheets if sheet in dfs]

    print(f"\nHojas encontradas (existing_sheets): {existing_sheets}")
    print(f"Cantidad de hojas encontradas: {len(existing_sheets)}")

    # Only create temp file if there are existing sheets to include
    if existing_sheets:
        print("\n✓ Se encontraron hojas válidas. Procediendo a crear temp file y enviar email...")
    else:
        print("\n✗ NO se encontraron hojas válidas. NO se enviará el email PowerAutomate")
        print("="*70 + "\n")
    ...
    print("\n" + "="*60)
    print("DEBUG: Mensaje de Slack (NO ENVIANDO)")
    print("="*60)
    print(mensaje)
    print("="*60 + "\n")
    # Original Slack code commented for debugging
    # slack_payload = {
